...
ObsRadianceCRTM::ObsRadianceCRTM(const ioda::ObsSpace & odb, //this is a constructor of the class ObsRadianceCRTM
                                 const Parameters_ & parameters)
  : ObsOperatorBase(odb), keyOperRadianceCRTM_(0), //call of base constructor
    odb_(odb), varin_(), parameters_(parameters),  // here varin_ is empy, created by the default construtor of oops::Variables
    ...
    sample_weights_()
{
  ...
  ufo_radiancecrtm_setup_f90(keyOperRadianceCRTM_, parameters_.toConfiguration(), //parameters converted to fortran
                             channels_list.size(), channels_list[0], midPointJulday,
                             varin_, odb.comm());
    //varin_ will be filled by a fortran routine(calling a c++ function) according to the CRTM config
